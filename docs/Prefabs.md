...
- In `SandboxGame`, prefabs are loaded from `Assets/Prefabs/*.prefab.json` and registered in the `DictionaryAssetProvider` by filename (e.g., `Assets/Prefabs/Player.prefab.json` -> `"Player"`).
